...
export const Random = {
  ...
  pick: (arr = []) => (Array.isArray(arr) && arr.length ? faker.helpers.arrayElement(arr) : null),
};

function resolvePlaceholder(token) {
...
function parseRange(spec) {
  ...
  if (m) {
    return randomInt(parseInt(m[1], 10), parseInt(m[2], 10));
  }
  return 1;
}

function mockNode(node) {
  if (Array.isArray(node)) {
    return node.map((item) => mockNode(item));
  }
