...
let reloadCallbacks: ((loadedFiles: string[]) => void)[] = [];
 ...
 */
export function watchEnvFiles(callback?: (loadedFiles: string[]) => void) {
  if (callback) {
 ...
 */
export function getEnv(key: string, defaultValue: any = undefined) {
 ...
 */
export function initEnvLoader(options: { watch?: boolean, onReload?: (loadedFiles: string[]) => void } = {}) {
